...
class LSTM(BaseModel):
        ...
        self.model = tf.keras.models.Sequential(
            [
                tf.keras.layers.LSTM(
                    256,
                    input_shape=[parameters["n_steps_in"], parameters["n_features"]],
                ),
    ...
    def windowed_dataset(
        self, series, window_size=168, batch_size=32, shuffle_buffer=1000
    ):
    ...
    def save_model(self, save_dir_model):
        ...
        save_file = os.path.join(
            dir_cp, os.path.basename(save_dir_model).split(".")[0] + ".ckpt"
        )
