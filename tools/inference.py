...
def get_parser():
    ...
    parser.add_argument("--inference_method", default="kriging", help="Available methods: knn, kriging, idw")
    ...
    for k_dir in tqdm(os.listdir(test_dir)):
        ...
        for model_name in all_models:
            ...
            for file_name in os.listdir(os.path.join(test_dir, k_dir)):
                ...
                if args.drop_empty == True and check_all_null(full_file_name):
            ...
            for idx, target_station in enumerate(target_stations):
                ...
                for today in data.columns[4:]:
                    ...
                    if args.inference_method == "knn":
                        ...
                        data.loc[data['station'] == target_station, today] = forecast_day_based_on_knn(X, y, target_feature, n_neighbor)
                    elif args.inference_method == "kriging":
                        data.loc[data['station'] == target_station, today] = forecast_day_based_on_kriging(X, y, target_feature)
                    elif args.inference_method == "idw":
                        data.loc[data['station'] == target_station, today] = forecast_day_based_on_idw(X, y, target_feature)
                    else:
