...
def get_parser():
    ...
    parser.add_argument("--phase", default="public-test", help="Available phases: public-test, private-test")
    parser.add_argument("--method", default="mean", help="Available methods: mean, interpolate")
    parser.add_argument("--train_folder_path", default="data/public-train/raw")
    parser.add_argument("--preprocessed_train_folder_path", default="data/public-train/preprocessed")
    parser.add_argument("--test_folder_path", default="data/public-test/raw")
    parser.add_argument("--preprocessed_test_folder_path", default="data/public-test/preprocessed")
...
def preprocess_data(args):
    ...
    need_to_preprocess_sub_dirs = ["input", "output"] if args.phase == 'public-test' else ["input"]
    for sub_dir in need_to_preprocess_sub_dirs:
        print(f"Preprocessing files in {sub_dir} in data train...")
        for input_file_path in tqdm(glob.glob(os.path.join(args.train_folder_path, f"{sub_dir}/*"))):
            file_name = os.path.basename(input_file_path)
            output_file_path = os.path.join(args.preprocessed_train_folder_path, f"{sub_dir}", file_name)
    ...
    copy(os.path.join(args.train_folder_path, "location.csv"), args.preprocessed_train_folder_path)
    ...
    if args.phase == 'public-test':
        for sub_dir in tqdm(glob.glob(os.path.join(args.test_folder_path, "input/*/"))):
            for input_file_path in glob.glob(sub_dir + "*"):
                ...
                output_file_path = os.path.join(args.preprocessed_test_folder_path, "input", sub_dir_name, file_name)
                fill_data(input_file_path, output_file_path, args.method)
        # copy `location.csv` file to the new preprocessed test folder
        copy(os.path.join(args.test_folder_path, "location.csv"), args.preprocessed_test_folder_path)
            ...
            for input_file_path in glob.glob(sub_dir + "*"):
                ...
                if file_name in ['location_input.csv', 'location_output.csv']:
                    copy(os.path.join(sub_dir, file_name),
                        os.path.join(args.preprocessed_test_folder_path, "input", sub_dir_name, file_name))
                    continue
                output_file_path = os.path.join(args.preprocessed_test_folder_path, "input", sub_dir_name, file_name)
                convert_to_true_format(input_file_path, args.phase)
                fill_data(input_file_path, output_file_path, args.method)
