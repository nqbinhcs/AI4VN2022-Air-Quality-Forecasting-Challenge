...
def get_parser():
    ...
    parser.add_argument("--saved_runs_folder", default="saved/", help="parent folder to store all runs log folder")
...
def train(args):
    ...
    copy(os.path.join(config['dataset']['train']['args']['path'], "location.csv"),
    ...
    for station_data_path in glob.glob(os.path.join(config['dataset']['train']['args']['path'], "input/*")):
        ...
        for model_name in config['models']:
            ...
            saved_model_path = os.path.join(saved_folder, "models", model_name, station_name + ".json")
